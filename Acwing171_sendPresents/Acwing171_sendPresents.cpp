 ...
 * 对比之前的均分明显少一个数量级
 *
 * 这里有一个问题就是如果leftEnd = n / 2 + 2会超时
 * 但是如果为n / 2就不会超时，这个点还没研究明白
 * @return int
 */
...
void dfsLeft(int u, int nowWeight)
{
    ...
    //不超过w的情况下，选第u个物品
    if ((LL)nowWeight + G[u] <= (LL)w)
        dfsLeft(u + 1, nowWeight + G[u]);
}
...
void dfsRight(int u, int nowWeight)
{
    ...
    if (u >= n)
    {
        ...
        {
            int mid = l + r + 1 >> 1;
            if (leftPresentWeightCombination[mid] <= w - nowWeight)
                l = mid;
            else
                r = mid - 1;
        }
